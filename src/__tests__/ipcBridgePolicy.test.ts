...
describe('renderer IPC bridge policy', () => {
  it('allows only known renderer invoke and send channels', () => {
    ...
    expect(isAllowedInvokeChannel('desktop:executeCommand')).toBe(false)
  ...
  it('allows stream event ids without allowing arbitrary event channels', () => {
    ...
    expect(isAllowedEventChannel('http:stream:event:renderer-http-stream-1-a_b')).toBe(true)
    expect(isAllowedEventChannel('http:stream:event:../../settings:load')).toBe(false)
